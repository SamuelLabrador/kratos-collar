...
int LSM6DSOXClass::begin()
{
  ...
  // See table 51.
  /**
   * LSM6DSOX_CTRL1_XL[7:4] (Output Data Rate)
   *
   * 0x0 -> Power-down
   * 0xB -> 1.6 Hz
   * 0x1 -> 12.5 Hz
   ...
   * 0x3 -> 52 Hz
   *
   * LSM6DSOX_CTRL1_XL[3:2] (full-scale selection)
   * 0x0 -> 2 g
   * 0x1 -> 16 g
   * 0x2 -> 4 g
   * 0x3 -> 8 g
   */
  writeRegister(LSM6DSOX_CTRL1_XL, 0x3A); // 52 Hz @ 4g
...
int LSM6DSOXClass::readAcceleration(uint16_t& x, uint16_t& y, uint16_t& z)
{
  uint16_t data[3];
  ...
  // Each register represents a range from -32,768 to 32,767
  // Acceleration = raw_value * (scale / 32,768)
...
{
  byte sampleRate = (readRegister(LSM6DSOX_CTRL1_XL) & 0xF0) >> 4;
  return sampleRate;
}

int LSM6DSOXClass::readGyroscope(uint16_t& x, uint16_t& y, uint16_t& z)
{
  uint16_t data[3];
