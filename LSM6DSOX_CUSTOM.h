...
class LSM6DSOXClass {
  public:
    ...
    // Accelerometer
    int readAcceleration(uint16_t& x, uint16_t& y, uint16_t& z); // Results are in g (earth gravity).
    ...
    // Gyroscope
    int readGyroscope(uint16_t& x, uint16_t& y, uint16_t& z); // Results are in degrees/second.
